...
parseAndProcess url s = parseCSV <$> trace ("parsing csv: of " ++ s ++ " from : " ++ url) CSV.parseCSV url s
...
parseActivities rows = [(pack p, (pack a, fes es)) | (p:a:es) <- tail rows, not (null p)]
  where
...
parseDate s =
  case read <$> splitOn "/" s of
    [m, d, y] -> Just Date {dateDay = d, dateMonth = toEnum (m - 1), dateYear = y}
    _ -> Nothing
